...
class SystemMonitor:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.config = config_manager.get_config() or {}
    ...
    async def _check_databases(self) -> Dict[str, Any]:
        ...
        databases_cfg = self.config.get("databases", {}) if self.config else {}

        # ---------------- PostgreSQL ----------------
        try:
            import psycopg2

            cfg = databases_cfg.get("postgresql", {})
            if not cfg:
                raise KeyError("Configuración PostgreSQL no encontrada")
        ...
        try:
            ...
            cfg = databases_cfg.get("redis", {})
            if not cfg:
                raise KeyError("Configuración Redis no encontrada")
    ...
    def export_metrics_csv(data: Dict[str, Any], path: Path) -> None:
        ...
        resources = components.get("resources", {}).get("details", {})
        ...
        with path.open("w", newline="", encoding="utf-8") as f:
            ...
            if resources:
                writer.writerow(["cpu_percent", resources.get("cpu", {}).get("percent"), data.get("timestamp")])
                writer.writerow(["memory_percent", resources.get("memory", {}).get("percent"), data.get("timestamp")])


# ---------------------------------------------------------------------
# CLI commands
# ---------------------------------------------------------------------
async def export_metrics_command(args) -> int:
    ...
    try:
        ...
        if args.format == "json":
            ...
            output.write_text(yaml.safe_dump(export_data, allow_unicode=True))
