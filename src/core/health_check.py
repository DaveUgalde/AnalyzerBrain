...
class SystemHealthChecker:
    ...
    async def _run_check(
        self, name: str, check_func: AsyncHealthCheck, critical: bool
    ) -> HealthCheckResult:
    ...
    async def _check_file_system(self) -> HealthCheckResult:
        ...
        required_dirs: List[Path] = [
            Path(
                self.config.get("storage.data_dir", "./data")
            ),  # Changed from Path("./data") to "./data"
            Path(
                self.config.get("storage.log_dir", "./logs")
            ),  # Changed from Path("./logs") to "./logs"
        ]
        ...
        for dir_path in required_dirs:
            dir_status: Dict[str, Any] = {"path": str(dir_path), "exists": False, "writable": False}
        ...
        details: Dict[str, Any] = {"directories": dirs_status, "errors": errors}
    ...
    def _check_file_system_sync(self) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                ...
                details={"data_dir": str(data_dir), "log_dir": str(log_dir)},
            )
        except Exception as e:
            return HealthCheckResult(
                name="file_system",
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": summary,
            "last_check": (
                max(r.timestamp for r in self.results.values()).isoformat()
                if self.results
                else None
            ),
    ...
    def print_detailed_report(self) -> str:
        ...
        summary = self._generate_summary()
        report_lines.append(
            f"Resumen: {summary['healthy']}✅ {summary['warnings']}⚠️ {summary['unhealthy']}❌ {summary['errors']}💥"
        )
        report_lines.append(
            f"Checks totales: {summary['total_checks']} | Tasa éxito: {summary['success_rate']}%"
        )
        report_lines.append("-" * 80)
